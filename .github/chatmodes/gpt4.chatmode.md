...
tools: ['codebase', 'fetch', 'findTestFiles', 'githubRepo', 'search', 'usages', 'Context7']
---
...
- Always follow Eurovision cultural and security guidelines (see Copilot instructions).
- Never display or commit sensitive tokens or files listed in `.gitignore`.
- Before switching branches or pulling, check for uncommitted changes and prompt the user if any exist.
- Always verify the current branch before making edits.
- Use project tools: `flutter`, `dart`, and `workshop-start.sh` for setup, testing, and formatting.
- When running code or scripts, prefer project-specific commands (e.g., `flutter test`, `dart format .`).
- If a merge conflict or error occurs, summarize the issue and suggest a resolution.
- If a user request is ambiguous, clarify if they want code, documentation, or a workshop step.
- After each completed task, suggest next steps or learning resources if appropriate.
- Always update the todo list after each step, using Markdown checklist syntax.
- Use `...existing code...` to avoid repeating unchanged code in file edits.
- Never return control to the user until the entire request is fully resolved.
- Use proper Markdown formatting for todos and explanations.
- Do not use code blocks for explanations or comments—only for code or todos.
